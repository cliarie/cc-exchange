...
class TradingBot: # TODO: There is still a bug for some reason where it doesn't receive the messages from the exchange after you run a second bot and after the second bot is done sending orders
  ...
  def send_orders(self, gateway_ip_addr : str, gateway_port : int):
    ...
    def parse_message(msg: bytearray):
      ...
      if msg_type == 87:
        ...
        resting_orders[accept_order.order_id] = Order(accept_order.order_id, accept_order.ticker, accept_order.price, accept_order.size, accept_order.mpid, accept_order.order_id, accept_order.timestamp, accept_order.side)
        self.inbound_msgs.put(accept_order)
      elif msg_type == 67: # C
        cancel_order = OrderCanceledOutbound()
        cancel_order.deserialize(msg)
        print(f"Received OUCH Cancel: {cancel_order}")
        self.inbound_msgs.put(cancel_order)
      elif msg_type == 69: # E
        executed_order = OrderExecutedOutbound()
        executed_order.deserialize(msg)
        print(f"Received OUCH Trade: {executed_order}")
        self.inbound_msgs.put(executed_order)
      elif msg_type == 74: # J
        rejected_order = OrderRejectedOutbound()
        rejected_order.deserialize(msg)
        print(f"Received OUCH Reject: {rejected_order}")
        self.inbound_msgs.put(rejected_order)
      else:
    ...
    try:
      while True:
        try:
          ...
          match topic_type:
            case "BBO5":
              bbo = MDF_BBO5("", 0, 0, 0, 0, 0, 0, 0, {}, {}) # TODO: Delete this
              bbo.deserialize(msg)
              # print(f"{topic}: {bbo}")
  ...
  def trade(self):
    # Always market make +-2 around 70
    position_limit = 100
    num_buy = 0
    num_bought = 0
    num_sell = 0
    ...
    try:
      while True:
        order_update = None
        try:
          order_update = self.inbound_msgs.get_nowait()
          if order_update is not None:
            if isinstance(order_update, OrderAcceptedOutbound):
              if order_update.side == "B":
                num_buy += order_update.size
              else:
                num_sell += order_update.size
            elif isinstance(order_update, OrderCanceledOutbound):
              if order_update.side == "B":
                num_buy -= order_update.decremented_shares
              else:
                num_sell -= order_update.decremented_shares
            elif isinstance(order_update, OrderExecutedOutbound):
              if order_update.side == "B":
                num_buy -= order_update.size
                num_bought += order_update.size
              else:
                num_sell -= order_update.size
                num_sold += order_update.size
            elif isinstance(order_update, OrderRejectedOutbound):
              print(f"Order rejected: {order_update}")
        except queue.Empty:
          pass
        if num_buy < position_limit:
          buy_order = OrderEntry("O", self.name, str(order_counter).rjust(10, '0'), "TPCF1010", "B", 68, position_limit - num_buy)
          num_buy += position_limit - num_buy # should aways equal position limit
          self.outbound_msgs.put(buy_order)
          order_counter += 1
        elif num_sell < position_limit:
          sell_order = OrderEntry("O", self.name, str(order_counter).rjust(10, '0'), "TPCF1010", "S", 72, position_limit - num_sell)
          num_sell += position_limit - num_sell
          self.outbound_msgs.put(sell_order)
          order_counter += 1
...
if __name__ == "__main__":
  ...
  parser.add_argument("--ip", type=str, help="IP address of the gateway and tickerplant", default="localhost")
  ...
  gateway_info = GatewayInfo(args.ip, args.port)
  tickerplant_info = TickerplantInfo(args.ip, 11000, ["TPCF1010-BBO5", "TPCF1010-ITCH", "GEN-TPC"])

  bot = TradingBot(args.name, None, gateway_info, tickerplant_info) # 192.168.56.20
